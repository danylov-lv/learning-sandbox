"""CP3 validator for the s10 capstone -- design memo + green re-run.
...
A design memo for a control-plane that no longer converges does not pass
this one either.
...
def main():
    ...
    if r1.returncode != 0:
        not_passed(
            f"CP1 no longer passes -- fix src/pipeline.py before finishing the writeup: "
    ...
    if r2.returncode != 0:
        not_passed(
            f"CP2 no longer passes -- fix src/pipeline.py before finishing the writeup: "
