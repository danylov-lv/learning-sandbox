...
Exception: the **toolkit track** (`toolkit/t1-ai-assisted-engineering`, `t2-modern-python-toolchain`, `t3-cli-data-toolkit`, `t4-git-advanced`) are tool-fluency modules — none has a `docker-compose.yml` or a host-port row. Each still ships its own `pyproject.toml` + committed `uv.lock`, `harness/common.py` (the module-17 pass/fail + doc-gate plumbing), and validators following the repo-wide "print `NOT PASSED: <reason>` and exit 1" rule. Their validators run the real local tools rather than a service: t1 exercises Claude Code mechanisms (`CLAUDE.md`, `.claude/agents`, hooks, `claude -p`, a stdio MCP server) plus a patch-review inversion; t2 shells out to `uv`/`ruff`/`mypy`/`pre-commit`; t3 to `jq`/`rg`/`fd`/`duckdb`/`hyperfine`/`parallel` and diffs stdout against an independently computed answer; t4 builds a throwaway scratch git repo per task under a gitignored `work/` dir and asserts the resulting repo state. A task that is *about* a `.claude/` config or a `CLAUDE.md` ships that config as a learner deliverable inside the task directory — it never touches the sandbox repo's own `.claude/` or root `CLAUDE.md`.

Exception: **`ci-meta`** is committed CI infrastructure for the repository itself, not a learner task — it has no unsolved stub, so unlike every other module its validator (`ci-meta/tests/validate.py`) PASSES on stock; that is expected, not a bug. Its scripts (`registry.py`, `detect_changes.py`, `checks.py`, `repo_guards.py`, `tests/validate.py`) are stdlib-only by design, since they run inside GitHub's `detect`/`guards` jobs against a bare runner python with no `uv sync` step (`run_module_ci.py` is the one exception, since it shells out to `uv`/`docker`). It has no `docker-compose.yml` and no host-port row — it doesn't run a service itself, it boots other modules' service containers on their behalf. See `ci-meta/README.md` for the full design and `ci-meta/.authoring/notes-live-verification.md` for what was verified locally.
