"""CP3 validator for the s11 capstone -- design memo + green re-run.
...
concurrency vocabulary this capstone is actually about -- backpressure,
semaphore, cancellation/timeout, retry, event loop), THEN re-runs CP1 and
...
A design memo for a scraper that no longer converges, or no longer holds
the concurrency cap, does not pass this one either.
...
corpus size; it exists only to catch a genuine hang (e.g. a deadlocked
queue or a retry loop with no cap).
...
def main():
    ...
    if r1.returncode != 0:
        not_passed(
            "CP1 no longer passes -- fix src/scraper.py before finishing the writeup: "
    ...
    if r2.returncode != 0:
        not_passed(
            "CP2 no longer passes -- fix src/scraper.py before finishing the writeup: "
