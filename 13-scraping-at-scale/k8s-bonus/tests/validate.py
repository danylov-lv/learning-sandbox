"""Validator for the module 13 k8s-bonus. Offline-first.

Always run (offline section, requires only `helm` on PATH):
  - `helm template` renders chart/ without error.
  - The rendered manifests contain:
      * a Deployment for the spider worker whose first container sets BOTH
        resources.requests AND resources.limits (cpu and memory) and defines
        BOTH a livenessProbe and a readinessProbe. If the resources are
        exactly the classic copy-paste defaults (cpu 100m, memory 128Mi),
        that is a WARNING with a pointed question, not a failure — measured
        numbers can legitimately land there, it's just rare;
      * a HorizontalPodAutoscaler (autoscaling/v2) whose spec.scaleTargetRef
        names that Deployment, with minReplicas, maxReplicas, and at least
        one entry in spec.metrics;
      * a PodDisruptionBudget whose selector matches the Deployment's pod
        labels and sets minAvailable or maxUnavailable.
  - `helm lint` exits 0.

Live section (OPTIONAL — auto-skipped with a notice, never a failure, when no
kind/k3d cluster is reachable via kubectl): a helm release of this chart is
installed somewhere in the cluster.
...
def _render_chart():
    result = _run(["helm", "template", "spider-platform-test", str(CHART_DIR)])
...
def check_deployment(docs, failures, warnings):
    ...
    if (
        ...
        warnings.append(
            ...
            "measure the spider (kubectl top / docker stats) across a crawl, or did muscle "
            "memory type that? If measured, the numbers belong in values.yaml comments; carry on."
        )
    for probe in ("livenessProbe", "readinessProbe"):
        if not container.get(probe):
            failures.append(f"Deployment container has no {probe}")
    return dep
...
def live_section():
    ...
    if result.returncode == 0:
        try:
            ...
            releases = []
    ours = [r for r in releases if str(r.get("chart", "")).startswith("spider-platform")]
    if ours:
        print(f"live: found installed release(s): {[r.get('name') for r in ours]}")
    else:
        print(
            "NOTICE: cluster reachable but no spider-platform release installed — the live "
            "check only reports, it does not fail. Install with `helm install` into your kind/"
            "k3d cluster to exercise it, then `kubectl scale deployment ... --replicas=N` and "
            "watch pods join/leave the worker pool."
        )
...
def main():
    ...
    if not docs:
        not_passed(
            "chart rendered zero manifests — templates/ is still the empty skeleton. Write the "
    ...
    passed(
        "chart renders, lints, and contains a resource-bounded spider Deployment with liveness "
        "and readiness probes, a matching HPA, and a matching PDB"
    )
