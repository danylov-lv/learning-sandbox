"""BUG: `upsert_observations` never commits.

The `conn.commit()` call at the end of `upsert_observations` is missing.
The insert/update happens inside an open transaction that is never
finalized, so the same connection can still read its own uncommitted
writes back (read-your-own-writes within one session), but any OTHER
connection against the same database sees nothing -- the write silently
never lands as far as the rest of the system is concerned. Looks fine in a
test that only ever uses one connection; breaks the moment a test opens a
second connection to check durability.
"""
