"""Bug: `__len__` no longer purges/excludes expired entries -- it reports
the raw internal storage size, so an entry that has passed its TTL
deadline but has not yet been touched by `get`/`put` (which do purge) is
...
class TTLCache:
    ...
    def __len__(self) -> int:
        # BUG: missing self._purge_expired() -- counts expired-but-not-
        # yet-purged entries as live.
