"""MUTANT -- CatalogRepo.upsert_products no longer commits the transaction. Rows are visible to the same connection (uncommitted reads see their own writes) but never become durable for a different connection unless something else happens to commit.
