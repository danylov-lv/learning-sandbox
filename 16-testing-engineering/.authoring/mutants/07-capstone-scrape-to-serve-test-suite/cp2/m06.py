"""MUTANT -- ProductCache._key ignores the sku entirely and always returns the bare namespace prefix, so every product collides on the exact same Redis key -- caching one product overwrites the cached entry for every other product.
