"""MUTANT -- CatalogRepo.upsert_products conflicts on the surrogate 'id' column instead of the business key 'sku'. Since 'id' is never part of the INSERT's column list (it's always a fresh auto-generated value), this ON CONFLICT target never actually matches -- upsert_products silently stops being idempotent and inserts a duplicate row every time the same sku is upserted again.
