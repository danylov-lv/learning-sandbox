"""MUTANT -- make_app's GET /products/{sku} route calls cache.get(sku) but discards the result instead of returning it on a cache hit -- every request falls through to repo.get_by_sku, so the cache has no observable effect on responses even though cache.set is still called.
...
def make_app(repo: CatalogRepo, cache: ProductCache) -> FastAPI:
    ...
    def get_product(sku: str):
        cache.get(sku)  # read result intentionally discarded
