"""MUTANT -- ProductCache.set no longer passes an expiry to Redis -- cached entries never expire (no TTL at all) instead of expiring after ttl/default_ttl seconds.
...
class ProductCache:
    ...
    def set(self, sku: str, product: dict[str, Any], ttl: int | None = None) -> None:
        ...
        self._client.set(self._key(sku), json.dumps(product))
