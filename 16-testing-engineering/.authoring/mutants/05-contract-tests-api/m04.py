"""BUG: the `next_cursor` presence check is inverted -- it comes back
`null` on a MID-STREAM page (silently truncating pagination, a consumer
walking cursors stops after the first page and never sees the rest of the
catalog) and a bogus non-null value on the actual last page.
"""
...
def make_app() -> FastAPI:
    ...
    async def list_products(
        ...
        next_cursor = str(page[-1]["id"]) if (page and not has_more) else None
