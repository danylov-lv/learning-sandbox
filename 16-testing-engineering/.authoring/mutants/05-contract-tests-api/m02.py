"""BUG: `GET /products/{id}` for a missing id returns `200` with a null
body instead of `404` with the error envelope -- callers checking only
`status_code == 200` never notice the product doesn't exist.
"""
