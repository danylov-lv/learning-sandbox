"""BUG: the `Cache-Control` response header on `GET /products` was
dropped -- a consumer relying on the documented caching contract now
re-fetches on every call (or a caching proxy in front of it treats the
response as non-cacheable when it should be cacheable for 30s).
"""
