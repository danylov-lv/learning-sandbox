"""CP3 validator for 08-capstone-converge: design writeup + CP1/CP2 re-check.

Checks:
  - DESIGN.md exists at the task root (copied from src/DESIGN_TEMPLATE.md
    and filled in) and contains all five required section headings.
  - DESIGN.md's content, minus the template's own text, is at least 2000
    characters (a length floor, not a quality bar -- it exists to keep
    "filled in" honest, not to reward padding).
  - CP1 (validate_cp1.main) and CP2 (validate_cp2.main) still pass -- a
    design writeup for a pipeline that no longer converges isn't a passing
    capstone.
...
def check_design_doc():
    if not TEMPLATE_PATH.exists():
        not_passed(f"template not found at {TEMPLATE_PATH} -- task scaffold is broken")
    if not DESIGN_PATH.exists():
        not_passed(
            f"DESIGN.md not found at {DESIGN_PATH} -- copy src/DESIGN_TEMPLATE.md to the task "
            "root and fill it in"
        )
...
def main():
    ...
    try:
        ...
        if e.code != 0:
            not_passed("CP2 no longer passes -- fix the pipeline before finishing the writeup")
