"""Shared pass/fail plumbing and doc/estimate checks for toolkit/t4-git-advanced
validators. Copied verbatim from `17-system-design/harness/common.py` — see
that module if this file drifts and needs reconciling.
...
t4's repo-state tasks (01-04) only use `guarded`, `not_passed`, and
`passed`. Task 05 (the writeup) additionally uses the doc-gate family:
`read_doc`, `parse_sections`, `parse_subsections`, `check_sections`,
`check_keywords`, `check_quantitative`, `check_answers`. The capacity-model
family (`load_workload`, `check_close`, `import_estimate`,
`check_estimate_module`) is unused here but kept for parity with the
source file.
"""
