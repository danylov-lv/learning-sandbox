"""CP3 validator for the s12 capstone -- design memo + green re-run.

Checks DESIGN.md is filled in (all six required section headings present,
each with real content beyond the shipped '(fill in' placeholder, at least
MIN_SECTION_CONTENT characters, and the document as a whole grounded in the
vocabulary this capstone is actually about -- keyset/cursor pagination,
cache invalidation, atomicity, token rotation, parametrized queries, graceful
degradation), THEN re-runs CP1 and CP2 as SUBPROCESSES (`uv run python
tests/validate_cp1.py` / `validate_cp2.py`, from this task's directory) and
requires both to exit 0. A design memo for a service that no longer
converges, or that regressed on any of CP2's hardening, does not pass this
one either.
...
600s is far beyond what either checkpoint should ever need against this
...
# these are meant to prove the memo is grounded in what this capstone is
# actually about, not scattered evenly.
REQUIRED_KEYWORDS = [
    ("keyset/cursor pagination", ["keyset", "cursor"]),
    ("cache invalidation", ["invalidat"]),
    ("atomicity", ["atomic"]),
    ("token rotation", ["rotat"]),
    ("parametrized query", ["parametriz", "parameteriz"]),
    ("graceful degradation", ["degrad"]),
]

CP_TIMEOUT = 600  # generous outer safety wrapper -- see module docstring
...
def main():
    ...
    if r1.returncode != 0:
        not_passed(
            "CP1 no longer passes -- fix src/app.py before finishing the writeup: "
    ...
    if r2.returncode != 0:
        not_passed(
            "CP2 no longer passes -- fix src/app.py before finishing the writeup: "
