...
QUERY_FILE = MODULE_ROOT / "queries" / "q06.sql"
...
def main():
    ...
    try:
        ...
        if not trgm_installed():
            reason = (
                "the pg_trgm extension is not installed, so no index can support "
                "a leading-wildcard ILIKE pattern -- extension missing, that is "
                "part of the fix"
            )
        else:
            reason = str(e)
    ...
    try:
        ...
        print("PASS  Bitmap Index Scan present (substring filter is index-driven)")
    except PlanAssertionError as e:
        print(f"FAIL  {e}")
